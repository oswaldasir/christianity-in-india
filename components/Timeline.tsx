...
export const Timeline = ({ onImageClick }: { onImageClick: (item: Item) => void }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8 pb-24 pt-12">
      {/* Overview Section */}
      <section id="overview" className="mb-16 mt-4 p-8 md:p-12 rounded-2xl bg-white/40 backdrop-blur-md border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.03)] relative overflow-hidden">
        <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-[#9a6418] via-[#1976a8] to-[#8f3565]"></div>
      ...
      {/* Control Panel */}
      <section className="mb-20 p-6 rounded-xl bg-white/60 backdrop-blur-xl border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.03)] relative z-30">
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-slate-600 text-sm mt-2">Search across the 60 entries, filter by role, or jump directly to an era.</p>
          </div>
        </div>
      ...
      {/* People Index */}
      <section id="people" className="mb-16 mt-8 relative z-20">
        <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-6">Index of Historical Figures (60)</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 p-6 md:p-8 rounded-2xl bg-white/40 backdrop-blur-md border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.03)]">
          {allItems.map(person => (
            <a 
              key={person.id}
              href={`#person-${person.id}`}
              className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-white/80 hover:shadow-sm transition-all border border-transparent hover:border-white group"
            >
              <img 
                src={person.image} 
                alt="" 
                className="w-10 h-10 rounded-full object-cover shadow-sm bg-slate-50 border border-slate-100" 
                onError={(e) => { e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(person.name)}&background=F1F5F9&color=475569&size=100&bold=true` }}
              />
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-slate-800 truncate group-hover:text-[#1976a8] transition-colors">{person.name}</div>
                <div className="text-[10px] text-slate-500 truncate uppercase tracking-wider">{person.year}</div>
              </div>
            </a>
          ))}
        </div>
      ...
      {filteredEras.length > 0 && (
        <div id="eras" className="mb-16 mt-8">
...
const TimelineCard: React.FC<{ item: Item, index: number, compact: boolean, onImageClick: (item: Item) => void }> = ({ item, index, compact, onImageClick }) => {
  ...
  return (
    <div id={`person-${item.id}`} className={`relative flex w-full mb-10 group ${isLeft ? 'md:justify-start' : 'md:justify-end'}`}>
      {/* Dot on the line */}
